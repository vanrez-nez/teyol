...
import type { CompactionPreparation, CompactionResult } from "#shell/runtime/compaction/index.js";
...
export interface ExtensionUIContext {
	...
	setWidget(
		key: string,
		content: ((tui: TUI, theme: any) => Component & { dispose?(): void }) | undefined,
	...
	setFooter(
		factory:
			| ((tui: TUI, theme: any, footerData: any) => Component & { dispose?(): void })
			| undefined,
	): void;

	/** Set a custom header component (shown at startup, above chat), or undefined to restore the built-in header. */
	setHeader(factory: ((tui: TUI, theme: any) => Component & { dispose?(): void }) | undefined): void;
	...
	/** Get current details expansion state. */
	getDetailsExpanded(): boolean;

	/** Set details expansion state. */
...
export interface ToolDefinition<TParams extends TSchema = TSchema, TDetails = unknown, TState = any> {
	...
	parameters: TParams;
	/** Controls whether the assistant tool timeline block renders the standard colored shell or the tool renders its own framing. */
	...
	renderCall?: (args: Static<TParams>, theme: any, context: ToolRenderContext<TState, Static<TParams>>) => Component;
...
export interface ToolCallEvent extends ToolCallEventBase {
	toolName: string;
...
export type GetCommandsHandler = () => any[];
