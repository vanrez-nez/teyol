...
const validateModelsConfig = TypeCompiler.Compile(ModelsConfigSchema);
...
function mergeCompat(
  ...
  const merged = { ...base, ...override } as OpenAICompletionsCompat;
...
export class ModelRegistry {
  ...
  private discoveryContext: DiscoveryContext = { overrides: new Map(), modelOverrides: new Map() };
  private dynamicRefreshPromise: Promise<void> | undefined;
  ...
  async refreshDynamic(): Promise<void> {
    if (this.dynamicRefreshPromise) {
      return this.dynamicRefreshPromise;
    }

    this.dynamicRefreshPromise = this.discoverModels().finally(() => {
      this.dynamicRefreshPromise = undefined;
    });
    return this.dynamicRefreshPromise;
  }
  ...
  private loadModels(): void {
    ...
    this.discoveryContext = { overrides, modelOverrides };
  ...
  private loadBuiltInModels(
    ...
    return getProviders().flatMap((provider) => {
      ...
      return models.flatMap((m) => {
        ...
        } else if (resolvedBaseUrl && resolvedBaseUrl !== model.baseUrl) {
          model = {
            ...model,
            baseUrl: resolvedBaseUrl,
          };
  ...
  private loadCustomModels(modelsJsonPath: string): CustomModelsResult {
    ...
    try {
      ...
      if (!validateModelsConfig.Check(parsed)) {
        const errors =
          Array.from(validateModelsConfig.Errors(parsed))
            .map((error: any) => `  - ${formatValidationPath(error)}: ${error.message}`)
            .join("\n") || "Unknown schema error";
  ...
  private parseModels(config: ModelsConfig): Model<Api>[] {
    ...
    for (const [providerName, providerConfig] of Object.entries(config.providers)) {
      ...
      for (const modelDef of modelDefs) {
        ...
        const baseUrl =
          modelDef.baseUrl ?? resolveProviderBaseUrl(providerName, providerConfig.baseUrl) ?? builtInDefaults?.baseUrl;
        if (!baseUrl) continue;
  ...
  private async discoverModels(): Promise<void> {
    ...
    const discoveredByProvider = await Promise.all(
      getProviders().map(async (provider) => {
        const metadata = getProviderMetadata(provider);
        const discoveryType = metadata?.discovery?.type;
        if (!discoveryType) return [];

        const providerOverride = this.discoveryContext.overrides.get(provider);
        const baseUrl = resolveProviderBaseUrl(provider, providerOverride?.baseUrl);
        if (!baseUrl) return [];

        try {
          if (discoveryType === "openrouter") {
            return await this.discoverOpenRouterModels(provider, baseUrl);
          }
          if (discoveryType === "ollama") {
            return await this.discoverOllamaModels(provider, baseUrl);
          }
        } catch (error) {
    ...
    for (const discoveredModels of discoveredByProvider) {
  ...
  private applyDiscoveryOverrides(models: Model<Api>[]): Model<Api>[] {
    return models.map((model) => {
      const providerOverride = this.discoveryContext.overrides.get(model.provider);
      const perModelOverride = this.discoveryContext.modelOverrides.get(model.provider)?.get(model.id);
      let nextModel = model;

      if (providerOverride) {
        nextModel = {
          ...nextModel,
          baseUrl: providerOverride.baseUrl ?? nextModel.baseUrl,
          compat: mergeCompat(nextModel.compat, providerOverride.compat),
        };
      }

      if (perModelOverride) {
        nextModel = applyModelOverride(nextModel, perModelOverride);
      }
  ...
  private async discoverOpenRouterModels(provider: KnownProvider, baseUrl: string): Promise<Model<Api>[]> {
    ...
    return (payload.data ?? [])
      .filter((model): model is NonNullable<typeof payload.data>[number] & { id: string } => !!model.id)
      .map((model) => {
        const input = model.architecture?.input_modalities?.includes("image") ? ["text", "image"] : ["text"];
        ...
        return {
          ...
          reasoning: true,
          input,
          cost: { input: promptCost || 0, output: completionCost || 0, cacheRead: 0, cacheWrite: 0 },
  ...
  private async discoverOllamaModels(provider: KnownProvider, baseUrl: string): Promise<Model<Api>[]> {
    const response = await fetch(`${baseUrl.replace(/\/+$/, "")}/tags`, {
      headers: await this.getDiscoveryHeaders(provider, baseUrl),
    });
    if (!response.ok) {
      throw new Error(`${response.status} ${response.statusText}`);
    }
    ...
    const payload = (await response.json()) as {
      models?: Array<{ name?: string; model?: string; details?: { family?: string; families?: string[] } }>;
    };
      ...
      .map(
        (id) =>
          ({
            ...
            provider,
            baseUrl,
            reasoning: /deepseek|qwen3|gpt-oss/i.test(id),
            input: ["text"],
  ...
  private applyProviderConfig(providerName: string, config: ProviderConfigInput): void {
    ...
    if (config.models && config.models.length > 0) {
      ...
      for (const modelDef of config.models) {
        const api = modelDef.api || config.api;
        this.storeModelHeaders(providerName, modelDef.id, modelDef.headers);
        const baseUrl = modelDef.baseUrl ?? resolveProviderBaseUrl(providerName, config.baseUrl) ?? config.baseUrl!;
      ...
      this.models = this.models.map((m) => {
        ...
        return {
          ...m,
          baseUrl: resolveProviderBaseUrl(providerName, config.baseUrl) ?? m.baseUrl,
        };
      });
    }
  }
