...
function parseThemeJson(label: string, json: unknown): ThemeJson {
	if (!TypeCompiler.Compile(ThemeJsonSchema).Check(json)) {
		const errors = Array.from(TypeCompiler.Compile(ThemeJsonSchema).Errors(json));
		const missingColors = new Set<string>();
		const otherErrors: string[] = errors.map((error: any) => `  - ${error.instancePath}: ${error.message}`);
...
const THEME_KEY = Symbol.for("@teyol/theme");
