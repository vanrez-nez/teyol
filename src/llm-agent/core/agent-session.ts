...
import type { AgentMessage, AgentState, AgentTool, ThinkingLevel } from "#agent/types.js";
...
import {
  ...
  type TurnEndEvent,
  type TurnStartEvent,
} from "#llm-agent/core/extensions/index.js";
import { emitSessionShutdownEvent } from "#llm-agent/core/extensions/runner.js";
import { wrapRegisteredTools } from "#llm-agent/core/extensions/wrapper.js";
...
export class AgentSession {
  ...
  private _installAgentToolHooks(): void {
    this.agent.beforeToolCall = async ({ toolCall, args }: any) => {
    ...
    this.agent.afterToolCall = async ({ toolCall, args, result, isError }: any) => {
      const runner = this._extensionRunner;
  ...
  private _getUserMessageText(message: Message | CustomMessage): string {
  ...
  /** All messages including custom types like  */
   ...
   * - Handles extension commands (registered via teyol.registerCommand) immediately, even during streaming
  ...
  async prompt(text: string, options?: PromptOptions): Promise<void> {
    ...
    try {
      ...
      // Extension commands manage their own LLM interaction via teyol.sendMessage()
  ...
  private async _cycleAvailableModel(direction: "forward" | "backward"): Promise<ModelCycleResult | undefined> {
    await this._modelRegistry.refreshDynamic();
  ...
  private async _checkCompaction(assistantMessage: AssistantMessage, skipAbortedCheck = true): Promise<void> {
    ...
    // compaction boundary. This prevents a stale pre-compaction usage/errors
  ...
  private _bindExtensionCore(runner: ExtensionRunner): void {
    const getCommands = (): SlashCommandInfo[] => {
      const extensionCommands: SlashCommandInfo[] = runner.getRegisteredCommands().map((command: any) => ({
    ...
    runner.bindCore(
      {
        ...
        sendUserMessage: (content: string | (TextContent | ImageContent)[], options?: any) => {
        ...
        setActiveTools: (toolNames: string[]) => this.setActiveToolsByName(toolNames),
   ...
   */
  async exportToHtml(outputPath: string, themeName = "teyol-dark"): Promise<void> {
